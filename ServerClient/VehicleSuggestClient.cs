...
namespace HelpAPIs
{
    internal class VehicleModels 
    {
        public List<CarModelSearch> TruckModels { get; set; }
        public List<CarModelSearch> TrailerModels { get; set; }
    }

    public class VehicleSuggestClient : IDataSuggest<TruckModelSuggestItem>, IDataSuggest<TrailerModelSuggestItem>
    {
        private string _storagePath = "models.txt";

        private List<CarModelSearch> _truckModels;
        private List<CarModelSearch> _trailerModels;

        private ILogger _logger;

        public VehicleSuggestClient(ILogger logger)
        {
            _logger = logger;
            Load();
        }
        ...
        {
            _truckModels = new List<CarModelSearch>()
            {
            ...
            _trailerModels = new List<CarModelSearch>()
            {
            ...
            VehicleModels models = new VehicleModels()
            {
                TruckModels = _truckModels,
                TrailerModels = _trailerModels
            };

            try
            {
                using (var file = File.OpenWrite(_storagePath))
                {
                    JsonSerializer.Serialize<VehicleModels>(file, models);
                }
            }
            catch (Exception ex)
            {
        ...
        private void Load() 
        {
            ...
            else 
            {
                try
                {
                    ...
                    VehicleModels models = JsonSerializer.Deserialize<VehicleModels>(str);

                    _truckModels = models.TruckModels;
                    _trailerModels = models.TrailerModels;
                }
                catch (Exception ex)
                {
                    _logger.Log(ex, ex.Message, LogLevel.Error);
        ...
        public Task<IEnumerable<TrailerModelSuggestItem>> SuggestAsync(string searchString)
        {
            return Task.FromResult(_trailerModels.Where(m => m.SearchInputs.Contains(searchString.ToLower())).Select(s => new TrailerModelSuggestItem() { TrailerModel = s.Standart }));
        }

        Task<IEnumerable<TruckModelSuggestItem>> IDataSuggest<TruckModelSuggestItem>.SuggestAsync(string searchString)
        {
            return Task.FromResult(_truckModels.Where(m => m.SearchInputs.Contains(searchString.ToLower())).Select(s => new TruckModelSuggestItem() { TruckModel = s.Standart }));
        }
    }
}
